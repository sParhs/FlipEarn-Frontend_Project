...
const ChatBox = () => {
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 flex z-100 items-center justify-center sm:p-4'>
      <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-2xl h-screen sm:h-[600px] flex flex-col'>
         ...
         <div className='bg-gradient-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
          ...
          <button onClick={()=>dispatch(clearChat())} className='m-4 p-1 hover:bg-white/20 hover:bg-opacity-20 rounded-lg transition-colors'>
            <X className='w-5 h-5'/>
          </button>
         </div>
         ...
         <div className='flex-1 overflow-y-auto p-4 space-y-4 bg-gray-100'>
            {isLoading ? (
                ...
                messages.map((message)=>(
                    <div key={messages.id} className={`flex ${message.sender_id === user.id ? "justify-end" : "justify-start"}`}>
                       <div className={`max-w-[70%] rounded-lg p-3 pb-1 ${message.sender_id === user.id ? "bg-indigo-600 text-white" : "bg-white border border-gray-200 text-gray-800"}`}>
                        ...
                        <p className={`text-[10px] mmt-1 ${message.sender_id === user.id ?"text-indigo-200" : "text-gray-400"}`}>
